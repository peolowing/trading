...
export function updateWatchlistStatus(input) {
  ...
  if (!trendOk && !inReclaimZone) {
    // Build detailed reason with specific failures
    const failures = [];
    if (close <= ema20) failures.push(`Pris (${close.toFixed(2)}) under EMA20 (${ema20.toFixed(2)})`);
    if (ema20 <= ema50) failures.push(`EMA20 (${ema20.toFixed(2)}) under EMA50 (${ema50.toFixed(2)})`);
    if (ema50_slope <= 0) failures.push(`EMA50 slope ${(ema50_slope * 100).toFixed(3)}% (behöver >0%)`);
    if (ema20_slope <= 0) failures.push(`EMA20 slope ${(ema20_slope * 100).toFixed(3)}% (behöver >0%)`);
    if (structure.higherLow !== true) failures.push(`Saknar högre låg-struktur`);

    const detailedReason = failures.length > 0
      ? `Trend bruten: ${failures.join('; ')}`
      : "Trend bruten (kräver pris > EMA20 > EMA50, positiva slopes och högre låg)";
  ...
  if (inReclaimZone && !baseTrendOk) {
    // Build detailed reason for reclaim failure
    const failures = [];
    if (ema20 <= ema50) failures.push(`EMA20 (${ema20.toFixed(2)}) under EMA50 (${ema50.toFixed(2)})`);
    if (ema50_slope <= 0) failures.push(`EMA50 slope ${(ema50_slope * 100).toFixed(3)}% (behöver >0%)`);
    if (ema20_slope <= 0) failures.push(`EMA20 slope ${(ema20_slope * 100).toFixed(3)}% (behöver >0%)`);
    ...
    const detailedReason = failures.length > 0
      ? `Trend för svag för reclaim: ${failures.join('; ')}`
      : "Trend för svag för reclaim (kräver EMA20 > EMA50 + positiva slopes + högre låg)";
